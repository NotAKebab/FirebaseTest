...
{
    [SerializeField]
    private GameObject _friendsListPanel;
    [SerializeField]
    private GameObject _friendPrefab; // Prefab para mostrar un amigo en la lista
    ...
    private void LoadFriendInfo(string friendId)
    {
        mDatabaseRef.Child("users").Child(friendId).GetValueAsync().ContinueWithOnMainThread(task =>
        {
            if (task.IsCompleted && task.Result.Exists)
            {
                ...
                friend.GetComponent<Friend>().SetFriendData(friendId, username);
            }
            else
            {
